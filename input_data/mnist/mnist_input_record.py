...
def inputs(data_dir,
  ...
  with tf.name_scope('input'):
    filename_queue = tf.train.string_input_producer(
        filenames, shuffle=(split == 'train'), seed=fix_seed)
    ...
    if split == 'train':
      batched_features = tf.train.batch(
          ...
          # min_after_dequeue=batch_capacity, seed=fix_seed)
    else:
      batched_features = tf.train.batch(
          features,
          batch_size=batch_size,
