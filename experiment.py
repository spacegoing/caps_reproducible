...
# My Flags
tf.flags.DEFINE_bool('fix_seed', True, 'Either train the model or test the model.')
tf.flags.DEFINE_bool('debug', True, 'Either train the model or test the model.')
...
def train_experiment(session, result, writer, last_step, max_steps, saver,
  ...
  if save_step > 0:
    saver.save(
        session, os.path.join(summary_dir, 'model.ckpt'), global_step=max_steps)
