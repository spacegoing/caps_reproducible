...
def _depthwise_conv3d(input_tensor,
  ...
  # input var values
  # input_dim,    1
  # output_dim,   32  number of capsules in this layer
  # input_atoms,  256
  # output_atoms, 8
  # todo: What is 20 * 20 or 6 * 6, namely height * width in one capsule

  with tf.name_scope('conv'): # 'conv_capsule1/conv'
    input_shape = tf.shape(input_tensor)
    ...
    # input_tensor: (128,1,256,20,20) (batch, , channels, img_height, img_width)
  ...
  with tf.variable_scope(layer_name): # layer_name = 'conv_capsule1'
    ...
    with tf.name_scope('routing'):
      ...
      biases_replicated = tf.tile(biases,
                                  [1, 1, votes_shape[2], votes_shape[3]], name="lc_tile")
      # (32,8,6,6)
      activations = _update_routing(
          votes=votes,
